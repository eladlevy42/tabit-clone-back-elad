...
exports.getReservationByRestaurantIdAndDate = getReservationByRestaurantIdAndDate;
...
async function addReservation(req, res) {
    const { tableId, restId, partySize, firstName, lastName, phoneNumber, email, notes, date, } = req.body;
    ...
    try {
        ...
        const [existingReservations] = await connection.query(`SELECT * FROM Reservations 
       WHERE tableId = ? 
       AND restId = ?
       AND (date > ? AND date < ?)`, [tableId, restId, startTime, endTime]);
        ...
        const [result] = await connection.query(`INSERT INTO Reservations (tableId, restId, partySize, firstName, lastName, phoneNumber, email, notes, date) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            tableId,
            restId,
            partySize,
        ...
        const [restaurantRows] = await connection.query(`SELECT * FROM Restaurants WHERE restId = ?`, [restId]);
...
function sendSMS(reservation, restaurant) {
...
async function getReservationByRestaurantIdAndDate(req, res) {
    ...
    try {
        ...
        const [rows] = await connection.query(`SELECT * FROM Reservations WHERE restId = ? AND DATE(date) = DATE(?)`, [restId, date]);
...
async function editReservation(req, res) {
    ...
    try {
        ...
        const [result] = await connection.query(`CALL edit_reservation(?, ?, ?, ?)`, [reservationId, tableId, newPartySize, newDate]);
...
async function deleteReservation(req, res) {
    ...
    try {
        ...
        const [result] = await connection.query(`DELETE FROM Reservations WHERE reservationId = ?`, [reservationId]);
...
async function getReservationById(req, res) {
    ...
    try {
        ...
        const [rows] = await connection.query(`CALL get_reservation_by_id(?)`, [reservationId]);
